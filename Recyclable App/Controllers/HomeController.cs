...
namespace Recyclable_App.Controllers
{
    public class HomeController : Controller
    {
        ...
        [HttpPost]
        public IActionResult ProcessForm(string type, decimal rate, decimal minKg, decimal maxKg)
        {
            try
            {
        ...
        public IActionResult DeleteConfirmed(int id)
        {
            try
            {
                ...
                {
                    foreach (var item in recyclableItems)
                    {
        ...
        public IActionResult ItemForm(int RecyclableTypeId, string Description, decimal Weight, decimal ComputedRate)
        {
            try
            {
                ...
                // Retrieve the recyclable types based on RecyclableTypeId
                var recyclableTypes = _context.RecyclableTypes
                    .Where(rt => rt.Id == RecyclableTypeId)
                    .ToList();

                if (recyclableTypes.Count == 0)
                {
                    TempData["Type"] = "Invalid recyclable type.";
                    return RedirectToAction("Index");
                }

                decimal min = recyclableTypes.First().MinKg;
                decimal max = recyclableTypes.First().MaxKg;
                ...
                {
                    TempData["Weight"] = $"Weight must be between {min} and {max}.";
                    return RedirectToAction("Index");
                }
